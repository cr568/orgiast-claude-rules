...
提案・設計・トレードオフ判断のすべてで、**user の手作業1回を最も高いコスト**として扱う。「1クリックだけ」「年数回だけ」を軽く見ない。アカウント数×プロバイダ数×PC数で積み上がるためである。

課金暴走・誤設定・データ消失などのリスクは、user の手作業で止めるのではなく、**Claude 側の監視・自動停止・DM通知で吸収**する設計にする。手作業を残す提案では「全体で年に何回発生するか」を数えて明示し、先にClaude側で吸収できない理由を書く。数えずに「1クリックだから」と正当化することは禁止。

ただし、§1.1 の **🛑 上限（人間の理解・同意・他AIの安全判断は減らさない）** はこの節より優先する。
...
手渡した作業を後で Claude 自身がやれた場合は、言われなくても毎回 `[再発防止]`（原因・対策・機械化）を書き、memory と hook のどちらかに落とす。
<!-- END: 手渡し品質ルール -->
...
- **コマンドを渡す前に「その端末で実行できるか」を確かめる（2026-09-16 実害でルール化）**。
  ...
  実害(2026-09-16): `gh pr merge` を「kimkon1011 でログイン済みの端末で」と条件付きで渡したところ、
  nishi@ が自分のPCで実行し `does not have the correct permissions` を2回踏んだ。
  **このPCの `gh` が nishiOrgiast 認証であることは、その前に `gh auth status` で確認済みだった**
  ＝分かっていたのに条件付きで渡したのが誤り。
  実行主体が自分でも相手でもない第三者なら、渡すのは**コマンドではなく「その人に送る文面」**にする。
  \`tools/manual-request-fullsteps-gate.mjs\` が機械検査する（コマンドを含むフェンスがある依頼で
  3点が揃っていないと block）。相手の端末が Windows か Mac か分からないときは、
  **推測で片方だけ書かず両方書く**か、先に1行で聞く。
...
完成済み指示書を探してから着手し、ChatGPT指定の画像デザインは生成結果に忠実に実装し、見せる資料はハイブリッド企画書モデル（Claude組版×Gemini写真×Genspark参照）で作る。
...
### 1.19.2 見せる資料はハイブリッド企画書モデルで作る（Claude組版 × Gemini写真ビジュアル × Genspark配置参照）（kim 2026-09-19 決定・全PC）

見せる資料はハイブリッド企画書モデル（Claude組版×Gemini写真×Genspark参照）。Claude単独版は写真で3回、Genspark単独版は数字・テキストで差し戻し、ハイブリッド v4 で承認された。実測（写真 / 余白 / 文字量）は Claude単独 3 / 3 / 2、Genspark単独 5 / 4 / 4（文字NG）、ハイブリッド v4 は全項目4〜5。

| 担当 | 役割 | やらないこと |
|---|---|---|
| Claude | 構成・原稿・文字と数字のHTML/CSS組版 | 写真の暗化・小配置・スクショ拡大 |
| Gemini画像モデル | 実写真2〜4枚を基に文字なしの大きな写真ビジュアルを合成 | 数字・表・図解・文字。社長パースがあればAIパースを作らず原図を直接表示 |
| Genspark AI Slides | 写真の面積比・余白・配置文法の参照 | 成果物をそのまま採用 |

対象は提案書・企画書・営業資料・プレゼン・デッキ・スライド・ブースプラン・パース入り資料など、見た目が評価対象になる資料。必ず `/design-deck` を使う。
対象外は学会1枚資料のような定型量産物・数表主体の実務資料。従来のHTML→PDFを使う。
写真への不満はGeminiで再合成し、文字・数字・表・余白への不満はClaudeが `pages.json` / CSSを直す。
まばら・設営中・床にゴミや資材・私的集合写真は不可。他社ロゴが大きければモザイク。Canvaは安定した写真投入経路がないため使わない。
詳細: `skills/design-deck/SKILL.md`、同梱 `pipeline/README.md`、`rules-extracted/design-deck-genspark.md`。
...
7. 拒否は操作単位。複数変更を束ねた操作や緩める方向の変更が拒否されても、手渡し前に目的の変更だけを最小単位で1回試す（例: `effortLevel` の1行 Edit）。障壁（hook・ブランチ・cooldown）を自分で取り除いた後も同じ操作を1回再試行する。同じ操作の言い換え・分割による迂回は引き続き禁止。user から明示の再指示があれば1回だけ再試行する。
...
`cost-improve-loop` は日次で `tools/provider-balance.mjs` を実行し、各社の残高・クレジット・当月消費を API/台帳で取得する。日次消費が直近7日平均の2倍超かつ $1 以上なら、該当レーンを24時間降格して kim に `spend_anomaly` DM。残高 $3 未満かつ自動チャージなしなら `balance_low` DM。残高 API がないものは **監視不能（台帳推定）** と明示し、0ドル扱いしない。全額を月次 `budget-status` に合算する。
